...
class _AuthScreenState extends ConsumerState<AuthScreen> {
  ...
  Future<void> _sendCode() async {
    ...
    if (!_isSignUp) {
      // Login: verify account exists before sending OTP
      setState(() => _loading = true);
      try {
    ...
    try {
      ...
      await AuthService.sendOtp(email, l10n.appName);
      if (mounted) {
        setState(() {
          _otpEmail = email;
          _step = _Step.otp;
  ...
  Future<void> _resendCode() async {
    ...
    try {
      final l10n = AppL10n(ref.read(localeProvider));
      await AuthService.sendOtp(_otpEmail, l10n.appName);
      if (mounted) {
        _otpCtrl.clear();
      ...
      children: [
        Center(
          child: Container(
            ...
            child: Icon(Icons.groups_rounded, color: c.primary, size: 42),
          ),
        ),
        ...
        Row(
          children: [
            ...
            Padding(
              ...
              child: Text(l10n.or, style: TextStyle(color: c.textMuted, fontSize: 13)),
            ),
            Expanded(child: Divider(color: c.border)),
          ],
        ),
